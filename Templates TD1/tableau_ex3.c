...
#define maketablenew(type) \
  ...
  tabletype(type) tablefunction(type,_delete)(tabletype(type) table) {\
    ...
    if (table->delete_data) \
      /* Liberation des éléments pointés si nécessaire*/\
  ...
  // Et tous les autres include et defininition de macro précédente
...
int main() { int i;
  table_double_t t1;
    // tableau de 10 reels
  t1=table_double_new(10,double_fprint,NULL,NULL);
  if (t1==NULL) {
    printf("Erreru allocation\n");
    exit(EXIT_FAILURE);
  }
    // On remplit avec des valeurs aléatoires
  for (i=0; i<t1->max_size; i++)
